...
historie:
  ...
  - datum: 2026-08-14
    notiz: "Corrections: replaced the per-source practical-takeaway boxes with a single box at the end of the article (plain language, drawn from the whole piece); removed the single-value chart for total sleep time (no chart for single values without spread); researched and added two boxplot-style range charts with real confidence intervals from Lee YJ et al. (2025) instead; extended the accuracy chart with a sleep/wake comparison row. Still draft, pending human review."
quellen:
  ...
  - aussage: "A meta-analysis of 24 studies and 798 participants found that consumer wearables underestimate total sleep time by about 16.9 minutes (95% CI −26.3 to −7.4) and sleep efficiency by about 4.7 percentage points (95% CI −7.1 to −2.3) versus polysomnography, while overestimating sleep latency by about 2.6 minutes (95% CI 0.6 to 4.5) and wake after sleep onset (WASO) by about 13.3 minutes (95% CI 4.5 to 22.0)."
    studientyp: "peer-reviewed"
...
praxisfazit:
  - "If deep-sleep and REM shares change little over the years in your own tracking data, that's normal — and a REM share that declines with age is an ordinary part of getting older, not a warning sign on its own."
  - "Seeing noticeably more or fewer than three to four sleep cycles on your tracker on a given night isn't a reason to worry by itself — night-to-night variation is large even in healthy people."
  - "A wearable can't look into your brain the way a sleep lab can — for persistent sleep problems, the app alone tells you nothing reliable about the cause, and that's when it's worth talking to a doctor."
  - "Trust your total sleep time and the trend across several nights more than the exact deep-sleep or dream-sleep percentage on any single night — that's where device accuracy varies the most."
affiliate: true
entwurf: true
offenePunkte:
  ...
  - "Confidence intervals for Lee YJ et al. (2025) were researched via Europe PMC abstract extraction (not the full PDF) — spot-check against the full text before release."
featured: true
istSaeule: true
...
17 minutes on average, and sleep efficiency by about 4.7 percentage points, while overestimating
sleep latency by about 2.6 minutes and wake after sleep onset (WASO) by about 13.3 minutes
(Lee YJ et al., 2025). Those are systematic biases in basic sleep parameters — before we even
get to the finer question of which sleep stage is currently underway. The charts below show not
just these averages but also how much the estimate varies across the 24 underlying studies
(95% confidence interval).

<Chart id="schlafphasen-abweichung-minuten" lang={frontmatter.sprache} />

<Chart id="schlafphasen-abweichung-effizienz" lang={frontmatter.sprache} />
...
detection is markedly more reliable than telling *which* sleep stage is currently underway —
the chart below shows that difference directly. For a medical assessment — say, a suspected
sleep disorder — none of these consumer devices substitute for a sleep-lab evaluation;
polysomnography remains the diagnostic gold standard that every figure cited here is measured
